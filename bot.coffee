...
window.begin = ->
  ...
  screenToGrid = (x, y) ->
    new Point(Math.floor(x / GRID_RES), Math.floor(y / GRID_RES))
  ...
  targetLoop = ->
    ...
    keys = []
    if mode == "seek"
      if point.x > myPoint.x then keys.push("right")
      if point.x < myPoint.x then keys.push("left")
      if point.y < myPoint.y then keys.push("up")
      if point.y > myPoint.y then keys.push("down")
    else if mode == "avoid"
      if point.x > myPoint.x then keys.push("left")
      if point.x < myPoint.x then keys.push("right")
      if point.y < myPoint.y then keys.push("down")
